...
freq_transformations = [
    datahandler.Spectrogram(args.spectrogram),
    datahandler.Logarithm()
]

train_transformations = [
    datahandler.Crop(3000),
    datahandler.RandomMultiplier(-1),
    datahandler.Fork({
...
train_set = datahandler.DataSet(
    'data/train.csv',
    load=datahandler.load_forked,
    transformations=train_transformations,
    path='data/',
    tokens='NAO')

test_set = datahandler.DataSet(
    'data/test.csv',
    load=datahandler.load_forked,
...
train_producer = torch.utils.data.DataLoader(
        ...
        num_workers=16, collate_fn=datahandler.batchify_forked)
