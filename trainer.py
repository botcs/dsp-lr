...
def accuracy(logit, target):
    ...
    F1 = torch.cat([F1[None, :], F1[None, :].mean(dim=1)[None]], dim=1)

    return F1


def ema_list(series, alpha=0.001):
    res = [series[0]]
    x = res[-1]
    for t in series[1:]:
        x = x + (t - x) * alpha
...
def evaluate(net, test_producer, gpu_id):
    ...
    for i, data in enumerate(test_producer, 1):
        ...
        if not isinstance(input, dict):
            input = Variable(data['x'].cuda(gpu_id))
        else:
            for k, v in input.items():
                input[k] = Variable(v.cuda(gpu_id))

        label = data['y']
        outputs = net(input)
        if i == 1:
            acc_sum = accuracy(outputs.data, label)
        else:
            acc_sum += accuracy(outputs.data, label)
...
class Trainer:
    ...
    def train(self, net, train_producer, test_producer, epochs=420,
        ...
        for epoch in range(self.highscore_epoch, epochs+1):
            ...
            for i, data in enumerate(train_producer, 1):
                ...
                label = Variable(data['y'].cuda(gpu_id))
                ...
                self.train_F1.append(
                    accuracy(outputs.data, data['y']))
                acc_sum += self.train_F1[-1]
                if i % (len(train_producer) // 20) == 0:
            ...
            if self.path and epoch % (epochs // 2) == 0:
                torch.save(net.state_dict(), self.path+'/state_dict')

            torch.save(self, self.path + '/' +'trainer')

            log_string = 'Train acc (F1):\n'
            log_vals = (acc_sum/i).tolist()[0]
            for i, val in enumerate(log_vals[:-1]):
                log_string += 'Class%1d: %.4f  ' % (i, val)
            log_string += 'mean: %.4f' % log_vals[-1]
            print(log_string, file=log, flush=True)
            ...
            if test_acc.tolist()[0][-1] > self.test_highscore:
                ...
                torch.save(net.state_dict(), self.path+'/state_dict_highscore')

            log_string = 'Test acc (F1):\n'
            log_vals = test_acc.tolist()[0]
            for i, val in enumerate(log_vals[:-1]):
                log_string += 'Class%1d: %.4f  ' % (i, val)
            log_string += 'mean: %.4f' % log_vals[-1]
            print(log_string, file=log, flush=True)
    ...
    def plot(self, ema_loss=.1, ema_train_f1=.1, ema_test_f1=.8, filename=None):
        ...
        fig1 = plt.figure()
        #plt.subplot(3, 1, 1)
        plt.plot(ema_list(losses, ema_loss))
        ...
        if len(self.class_weight) == 3:
            plt.plot(ema_list(th.cat(F1)[:, 0], alpha), label='N')
            plt.plot(ema_list(th.cat(F1)[:, 1], alpha), label='A')
            plt.plot(ema_list(th.cat(F1)[:, 2], alpha), label='O')
        #plt.plot(ema_list(th.cat(F1)[:, 3], alpha), label='~')
        elif len(self.class_weight) == 2:
            plt.plot(ema_list(th.cat(F1)[:, 0], alpha), label='Class1')
            plt.plot(ema_list(th.cat(F1)[:, 1], alpha), label='Class2')
        plt.plot(ema_list(th.cat(F1)[:, -1], alpha), label='Mean')
        ...
        if len(self.class_weight) == 3:
            plt.plot(ema_list(th.cat(test_F1)[:, 0], alpha), label='N')
            plt.plot(ema_list(th.cat(test_F1)[:, 1], alpha), label='A')
            plt.plot(ema_list(th.cat(test_F1)[:, 2], alpha), label='O')
        #plt.plot(ema_list(th.cat(F1)[:, 3], alpha), label='~')
        elif len(self.class_weight) == 2:
            plt.plot(ema_list(th.cat(test_F1)[:, 0], alpha), label='Class1')
            plt.plot(ema_list(th.cat(test_F1)[:, 1], alpha), label='Class2')
